...
console.log(greet("심선범"));
console.log(greet("심선범"));

//출력 결과는 같음, 내가 해야 하는 일 이외의 다른 일 처리 => 불순한 함수
//콘솔에 출력이 발생 => side Effect 발생!!

//입력 -> 출력 : 결과는 오직 입력에만 따라야함
//외부 변수 사용x : 외부 상태에 의존하면 순수하지 않음.
//예측 가능성 : 같은인자 -> 항상 같은 결과
...
브라우저 API 호출(setTimeoust, setInterval)
로컬 스토리지 접근
서버 요청(fetch,xhr,axios)
콘솔 출력(console.log)
상태 저장소에 쓰기/읽기 (contextAPI, Zustand, Redix, RTK)
